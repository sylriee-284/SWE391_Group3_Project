...
public class FirebaseChatService {
    ...
    private static final DateTimeFormatter CHAT_DATE_FORMATTER = DateTimeFormatter.ofPattern("dd-MM-yyyy HH:mm");
    ...
    private String formatTimestamp(Object ts) {
        ...
        if (ts instanceof Date) {
            LocalDateTime ldt = ((Date) ts).toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime();
            return CHAT_DATE_FORMATTER.format(ldt);
        }
        return ts.toString();
    }
    ...
    public List<ConversationDTO> getUserConversations(Long userId) throws ExecutionException, InterruptedException {
        validateUserId(userId, "userId");
    ...
    public String getOrCreateConversationByPartnerName(Long userId, String partnerName)
            throws ExecutionException, InterruptedException {
        Optional<User> userOpt = userRepository.findByUsername(partnerName);
        if (userOpt.isEmpty()) {
            throw new IllegalArgumentException("Không tìm thấy user với username: " + partnerName);
        }
    ...
    public String sendMessage(String conversationId, Long senderId, Long receiverId, String content)
        ...
        Map<String, Object> messageData = createMessageData(conversationId, senderId, receiverId, content);
    ...
    public List<MessageDTO> getOlderMessages(String conversationId, int pageSize, DocumentSnapshot lastDoc)
            throws ExecutionException, InterruptedException {
        validateConversationId(conversationId);
    ...
    public void sendWelcomeMessageAsync(Long userId) throws ExecutionException, InterruptedException {
        ...
        String welcomeMessage = "MMO Market System rất vui được đồng hành cùng bạn trên hành trình kiếm tiền online an toàn và minh bạch. "
                + "Chúng tôi cam kết cung cấp sản phẩm/dịch vụ chất lượng, giao dịch nhanh chóng và hỗ trợ tận tâm. "
                + "Nếu cần trợ giúp, đừng ngần ngại liên hệ với chúng tôi. "
                + "Chúc bạn trải nghiệm mua bán thật hiệu quả tại MMO Market System!";

        String messageId = sendMessage(conversationId, adminUserId, userId, welcomeMessage);
    ...
    private void enrichConversationWithPartnerInfo(ConversationDTO dto, String conversationId, Long currentUserId) {
        try {
            ...
            logger.warn("Failed to get partner info for conversation {}: {}", conversationId, e.getMessage());
    ...
    private void createNewConversation(DocumentReference conversationRef, Long userId1, Long userId2)
            throws ExecutionException, InterruptedException {
    ...
    private Map<String, Object> createMessageData(String conversationId, Long senderId, Long receiverId,
            String content) {
