                ...
                <form method="get" action="/admin/withdrawals" class="row g-3">
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="statusFilter" name="status" onchange="this.form.submit()">
                            ...
                            <option value="SUCCESS" ${selectedStatus == 'SUCCESS' ? 'selected' : ''}>Đã duyệt</option>
                            <option value="FAILED" ${selectedStatus == 'FAILED' ? 'selected' : ''}>Từ chối</option>
                                ...
                                <tbody>
                                    <c:forEach items="${withdrawals.content}" var="withdrawal">
                                        <tr>
                                            ...
                                            <td>
                                                <div>
                                                    <strong>${withdrawal.user.fullName}</strong><br/>
                                                    <small class="text-muted">${withdrawal.user.username}</small>
                                                </div>
                                            </td>
                                            <td>
                                                <strong>
                                            ...
                                            <td>
                                                ...
                                                <c:choose>
                                                    <c:when test="${fn:length(bankInfo) >= 4}">
                                                        ...
                                                        <c:set var="bankName" value="${fn:trim(bankInfo[1])}" />
                                                        <c:set var="accountNumber" value="${fn:trim(bankInfo[2])}" />
                                                        <c:set var="accountName" value="${fn:trim(bankInfo[3])}" />
                                                        <div>
                                                            <strong>${bankName}</strong><br/>
                                                            <small>${accountNumber}</small><br/>
                                                            <small class="text-muted">${accountName}</small>
                                                        </div>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <%-- Display raw note if format is incorrect --%>
                                                        <small class="text-muted">${withdrawal.note}</small>
                                                    </c:otherwise>
                                                </c:choose>
                                            </td>
                                            <td>
                                ...
                                <nav aria-label="Phân trang">
                                    <ul class="pagination mb-0 justify-content-center">
                                        ...
                                        <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
                                            <a class="page-link" href="?page=0&status=${selectedStatus}" aria-label="First">
                                                <span aria-hidden="true">&laquo;&laquo;</span>
                                            </a>
                                        </li>
                                        <!-- Previous Page -->
                                        <li class="page-item ${currentPage == 0 ? 'disabled' : ''}">
                                            <a class="page-link" href="?page=${currentPage - 1}&status=${selectedStatus}" aria-label="Previous">
                                        ...
                                        <c:if test="${currentPage + 2 < totalPages - 1}">
                                            <li class="page-item">
                                                <a class="page-link" href="?page=${totalPages - 1}&status=${selectedStatus}">${totalPages}</a>
                                            </li>
                                        </c:if>
                                        
                                        <!-- Next Page -->
                                        <li class="page-item ${currentPage == totalPages - 1 ? 'disabled' : ''}">
                                            <a class="page-link" href="?page=${currentPage + 1}&status=${selectedStatus}" aria-label="Next">
                                                <span aria-hidden="true">&raquo;</span>
                                            </a>
                                        </li>
                                        <!-- Last Page -->
                                        <li class="page-item ${currentPage == totalPages - 1 ? 'disabled' : ''}">
                                            <a class="page-link" href="?page=${totalPages - 1}&status=${selectedStatus}" aria-label="Last">
